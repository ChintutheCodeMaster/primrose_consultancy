...
function range(min: number, max: number) { return `${formatUSD(min)} – ${formatUSD(max)}`; }
function midpoint(min: number, max: number) { return Math.round((min + max) / 2); }
function countryFlag(value: string) { return COUNTRIES.find(c => c.value === value)?.flag ?? "🌍"; }
function countryName(value: string) { return COUNTRIES.find(c => c.value === value)?.label ?? value; }
function degreeName(value: string) { return DEGREE_TYPES.find(d => d.value === value)?.label ?? value; }
...
export default function TuitionCalculator({ onNavigate }: TuitionCalculatorProps = {}) {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data } = await supabase
        .from("cost_plans" as any)
        .select("*")
        .eq("student_id", identity.studentId)
        .order("created_at", { ascending: false })
        .limit(10);
      if (data) setSavedPlans((data as any[]) as CostPlan[]);
    })();
  }, [identity.studentId]);

  async function handleSavePlan() {
    if (!result || !isReady || !identity.studentId) return;
    ...
    setSaveStatus("saved");
    setSavedPlans(prev => [data as any as CostPlan, ...prev]);
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50/30 p-4 md:p-8 rounded-2xl">
                ...
                <div className="pt-1">
                  {saveStatus === "saved" ? (
                    ...
                    <Button
                      ...
                      disabled={saveStatus === "saving" || !identity.studentId}
              ...
              <Card className="shadow-sm border-border/60">
                <CardContent className="pt-5 pb-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <Button variant="outline" className="justify-start gap-2 h-auto py-3 text-left" onClick={() => onNavigate("scholarships")}>
                    ...
                    <Button variant="outline" className="justify-start gap-2 h-auto py-3 text-left" onClick={() => onNavigate("home")}>
                      <LayoutDashboard className="h-4 w-4 text-blue-500 shrink-0" />
                      <div>
                        <div className="text-xs font-semibold">Back to Home</div>
                        <div className="text-xs text-muted-foreground font-normal">Your workspace overview</div>
                      </div>
                    </Button>
                  </div>
                </CardContent>
